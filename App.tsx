  ...
  return (
    <div className="relative w-screen h-screen flex items-center justify-center font-sans">
      ...
      <div className={`absolute bottom-0 w-full h-12 border-t-2 border-white/20 flex items-center px-4 shadow-md z-0 ${isDarkMode ? 'bg-slate-800' : 'bg-gray-300'}`}>
          <div className="flex items-center space-x-2 bg-gradient-to-r from-green-600 to-green-500 text-white font-bold italic px-4 py-1 rounded-sm shadow-inner border border-green-800 cursor-pointer hover:brightness-110">
            Start
          </div>
          <div className={`ml-4 h-8 w-px ${isDarkMode ? 'bg-gray-600' : 'bg-gray-400'}`}></div>
          <div className={`ml-4 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Aduitor Active</div>
      </div>
      ...
      <div className="relative flex flex-col items-end gap-4 z-10 w-full max-w-4xl h-[80vh] pointer-events-none">
        ...
        {isOpen && (
          <div className={`pointer-events-auto w-96 border-2 rounded-lg shadow-2xl flex flex-col overflow-hidden animate-bounce-gentle transition-colors duration-300 relative ${windowClass}`} style={{ boxShadow: '10px 10px 0px rgba(0,0,0,0.2)' }}>
            ...
            {/* Mode Selector */}
            <div className={`p-2 border-b flex overflow-x-auto gap-2 scrollbar-hide transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-100 border-gray-300'}`}>
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs whitespace-nowrap transition-colors border ${
                            selectedMode === m.mode 
            ...
            <div className={`flex-1 overflow-y-auto p-4 space-y-4 max-h-60 transition-colors duration-300 ${chatAreaClass}`}>
              {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.sender === Sender.User ? 'items-end' : 'items-start'}`}>
                  <div 
                    className={`max-w-[85%] p-3 rounded-lg text-sm shadow-sm border ${
                   ...
                   <ul className="space-y-1">
                       {sortedTasks.map(t => (
                           editingId === t.id ? (
                               ...
                               <li key={t.id} className={`flex flex-col p-1 rounded border transition-all duration-300 ease-in-out group ${
                                    t.completed 
                                   ...
                                   <div className="flex items-center gap-2 text-sm">
                                       <div className="relative">
                                           <button 
                                             ...
                                             className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 transition-all duration-300 relative z-10 ${
                                                 t.completed 
                                       ...
                                       <button 
                                          ...
                                          className={`text-[9px] uppercase font-bold px-1 py-0.5 rounded border transition-colors ${
                                            t.priority === 'high' 
